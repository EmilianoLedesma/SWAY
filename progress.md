...
8. **"¿Olvidaste tu contraseña?" — pantalla construida, backend aún NO.** ✅ Parcial 2026-07-30: nueva `ForgotPasswordScreen.js` (input de correo + validación de formato client-side, sin llamada a API), registrada en `AppNavigator` dentro del stack no-autenticado, botón de `LoginScreen.js` ahora navega ahí (`719f523`). Submit muestra mensaje genérico "revisa tu correo" (no revela si el email existe) y regresa a Login. **Backend intencionalmente sin construir todavía** — sigue faltando el endpoint real de reset (token vía email u otro mecanismo) antes de que esto sea funcional de punta a punta.
