...
Última actualización: 2026-08-05 (deploy real verificado en vivo: commit `0ffa6d8` en ambos droplets, 9 contenedores en privado incluyendo Redis, 3 en público. Sesión de hoy: 4 bugs mobile + 1 feature vía subagent-driven-development completo — bug real de foto de avistamiento que nunca llegaba a otros clientes en tiempo real (ver sección 15), tarjeta de avistamiento completa tocable, chip de racha wireado a Actividad, selector cámara/galería al capturar foto, feedback visual en filtro de especie; suite `verify_pi_requirements.sh` post-deploy: 31 pass, 0 fail, 1 skip. Sesión anterior (2026-08-04): bounds server-side agregados — lat/lon de avistamiento, población estimada de especie (bug real reportado por overflow de INTEGER en Postgres), magic-number check en fotos, `terminos_aceptados` obligatorio en eventos, EmailStr en los 3 endpoints de login; el patrón de error leak (`detail=str(e)` exponiendo SQL crudo al cliente) corregido en 49 endpoints; simulación de ataques reales contra producción — SQL injection, JWT forjado, content-type spoofing, overflow — todos bloqueados; 300 requests concurrentes contra `/login` bloqueados 300/300 con 429)
...
**Sección 15 (nueva, 2026-08-04):** cubre en vivo, sin autenticación, los bounds server-side agregados esta sesión — latitud/longitud fuera de rango, fecha de avistamiento malformada, evento sin aceptar términos, `tipo_evento` inexistente (confirma 400 limpio, no 500 con SQL crudo), login con email malformado. Corrida completa post-deploy: **31 pass, 0 fail, 1 skip**.

**Simulación de ataques y saturación (2026-08-04, no scripteada — cuenta de prueba temporal, limpiada después):** overflow de entero en `poblacion_estimada` (bloqueado), inyección SQL real en `nombre_comun` (guardada como texto literal, tabla intacta — confirma ORM parametrizado), JWT forjado (401), content-type spoofing en upload de foto (bloqueado por magic-number real), 300 requests concurrentes contra `/login` → 300/300 bloqueados con `429` (rate limiting compartido por Redis sin ninguna filtración).
...
**Bug real encontrado y corregido — 2026-08-05, la foto de un avistamiento nunca llegaba a otros clientes en tiempo real.** El resto de los campos de un avistamiento (especie, notas, coordenadas) sí llegaban correctamente vía `avistamiento_created` a otros dispositivos conectados por WebSocket, pero la foto no. Causa raíz: la foto se sube en un **segundo request separado**, `POST /api/avistamientos/{id}/foto` (multipart, después de que el avistamiento ya fue creado con `POST /reportar-avistamiento`) — el evento `avistamiento_created` se publica en el primer request, cuando `foto_url` todavía es `NULL` en la base de datos, y el segundo endpoint nunca publicaba ningún evento propio. El dispositivo que sube la foto no lo notaba porque hace un refetch completo de la lista después de subir; cualquier *otro* dispositivo se quedaba con la tarjeta congelada sin foto para siempre. Corregido agregando `publish_event("avistamiento_updated", {"id", "foto_url"})` al final de `subir_foto_avistamiento` (`app/routers/estadisticas.py`), consumido en mobile con un merge puro nuevo (`patchById` en `realtimeMerge.js`) que parcha solo `hasPhoto`/`photoUrl` sin tocar el resto de la tarjeta. Desplegado en el droplet privado (`git pull` + rebuild real de `api1`/`api2`), verificado con la suite automatizada post-deploy. **Pendiente de confirmación con 2 dispositivos Expo Go reales** — la corrección está en producción pero el escenario de 2 dispositivos simultáneos no se volvió a probar en vivo esta sesión (sí se probó en la sesión original del feature, 2026-08-04, para el resto de los campos).
